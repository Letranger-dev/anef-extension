...
/**
 * Réinitialise les données liées à un dossier lors du passage à un autre dossier
 * (ex: l'utilisateur vérifie successivement son dossier puis celui de son conjoint
 * dans le même navigateur). Sans ce reset, history / stepDates / lastStatus
 * accumulent des entrées issues des deux dossiers → transitions incohérentes
 * (ex: 11.1 → 8.1) et contamination Supabase via sendManualStepDates.
 *
 * Conserve : credentials, clé de chiffrement, settings, autoCheckMeta, checkLog.
 * Efface : history, stepDates, lastStatus, lastCheck, apiData (partiel).
 */
export async function resetForNewDossier(newDossierId) {
  // Efface le backup sync pour éviter un restoreFromSync() post-reset qui
  // réinjecterait les données mélangées.
  try {
    await new Promise((resolve) => {
      chrome.storage.sync.remove([SYNC_KEYS.HISTORY, SYNC_KEYS.LAST_STATUS, SYNC_KEYS.STEP_DATES], resolve);
    });
  } catch { /* ignore */ }
  ...
  await set({
    ...
    // apiData minimal : garde uniquement le nouvel ID + timestamp
...
/** Vérifie si le statut a changé (compare date ET code statut) */
