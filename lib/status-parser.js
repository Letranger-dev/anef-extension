...
const STATUTS = {
  ...
  // Statut unifié « Contrôle SDANF » émis depuis le changement d'API (juillet 2026) :
  // l'API ne distingue plus « à affecter » (CAA) de « à effectuer » (CAE). Les anciens
  // dossiers gardent leur code granulaire (controle_a_affecter / controle_a_effectuer).
  "controle_sdanf": {
    phase: "Contrôle SDANF",
    explication: "Contrôle ministériel SDANF",
    etape: 9,
    rang: 900,
...
export const STEP_DEFAULTS = [
  ...
  { etape: 9, statut: 'controle_sdanf', label: 'SDANF — Contrôle ministériel', code: 'controle_sdanf', icon: '🏛️', sub: '9' },
  ...
  { etape: 10, statut: 'controle_transmise_pour_decret', label: 'Avis favorable, transmis pour décret', code: 'controle_transmise_pour_decret', icon: '🎉', sub: '10.1' },
  { etape: 10, statut: 'controle_en_attente_retour_hierarchique', label: 'Validation hiérarchique ministérielle', code: 'controle_en_attente_retour_hierarchique', icon: '👔', sub: '10.2' },
  { etape: 10, statut: 'controle_en_attente_signature', label: 'Attente signature ministérielle', code: 'controle_en_attente_signature', icon: '✍️', sub: '10.4' },
  { etape: 10, statut: 'a_verifier_avant_insertion_decret', label: 'Vérifications finales avant insertion', code: 'a_verifier_avant_insertion_decret', icon: '🔎', sub: '10.6' },
...
/**
 * Étapes dont les `rang` décrivent une VRAIE progression interne : les
 * sous-étapes s'y enchaînent dans l'ordre (7.1 → 7.2, 8.1 → 8.2 → 8.3…).
 * Ailleurs, `rang` ne sert qu'à ordonner l'affichage et comparer deux rangs
 * n'aurait aucun sens :
 *  - étape 12 : des ISSUES (naturalisé, refus, classement sans suite…) ;
 *  - étapes 3 et 5 : des branches de classement sans suite parallèles à
 *    l'instruction, dont un dossier peut revenir ;
 *  - étape 6 : `ea_demande_report_ea` (602) est un report d'entretien, pas une
 *    avancée — le dossier retourne légitimement en « date à fixer » (601).
 */
const ETAPES_A_RANG_PROGRESSIF = new Set([7, 8, 9, 10, 11]);

/**
 * Vrai si `nouveauCode` décrit un dossier MOINS avancé que `ancienCode`.
 *
 * Compare d'abord l'étape, puis — seulement dans les étapes à rang progressif —
 * la sous-étape. La frise de l'API (juillet 2026) est plus grossière que
 * l'ancien statut chiffré : elle ne connaît qu'un nœud par étape et re-décrit
 * un dossier connu en 8.2 comme « 8.1 ». Sans comparaison de rang, ce recul de
 * sous-étape passait le garde-fou (même étape) et écrasait le bon statut.
 *
 * Un code inconnu (étape 0) ne déclenche jamais de recul : on ne bloque pas sur
 * du vocabulaire non répertorié.
 */
export function isStatusRegression(ancienCode, nouveauCode) {
  const ancien = getStatusExplanation(ancienCode);
  const nouveau = getStatusExplanation(nouveauCode);
  if (!ancien.etape || !nouveau.etape) return false;
  if (nouveau.etape !== ancien.etape) return nouveau.etape < ancien.etape;
  if (!ETAPES_A_RANG_PROGRESSIF.has(ancien.etape)) return false;
  return !!ancien.rang && !!nouveau.rang && nouveau.rang < ancien.rang;
}

/**
 * Date à afficher pour l'étape 7 (entretien d'assimilation).
 *
 * L'historique date le passage au statut `ea_en_attente_ea`, c'est-à-dire
 * l'envoi de la CONVOCATION — pas l'entretien lui-même. Le rendez-vous réel
 * (`date_rdv` de l'API) fait donc foi dès qu'il est passé. Tant qu'il est à
 * venir, on garde la date de convocation : elle décrit correctement « depuis
 * quand le dossier est à cette étape ».
 *
 * @param {string|null} dateEntretien - date du rendez-vous (API)
 * @param {string|null} dateConvocation - date du statut, historique ou courante
 */
export function resolveEntretienDate(dateEntretien, dateConvocation) {
  if (!dateEntretien) return dateConvocation || null;
  if (!dateConvocation) return dateEntretien;
  const rdv = new Date(dateEntretien);
  if (isNaN(rdv)) return dateConvocation;
  return rdv.getTime() > Date.now() ? dateConvocation : dateEntretien;
}

/** Formate une durée en jours (convention : 1 an = 365 jours, 1 mois = 30 jours) */
export function formatDuration(jours) {
  ...
  if (jours < 365) {
    ...
    return `${mois} mois, ${reste} j`;
  }

  const annees = Math.floor(jours / 365);
  const resteAnnee = jours % 365;
  const mois = Math.floor(resteAnnee / 30);
  const joursRestants = resteAnnee % 30;

  const parts = [`${annees} an${annees > 1 ? 's' : ''}`];
  if (mois > 0) parts.push(`${mois} mois`);
  if (joursRestants > 0) parts.push(`${joursRestants} j`);
...
/** Nombre de jours calendaires entre deux dates (fin − début), fuseau français.
 *  Sert à figer une durée quand la procédure est terminée (ex. dépôt → décret). */
export function daysBetween(startStr, endStr) {
  ...
  try {
    ...
    const fmt = new Intl.DateTimeFormat('fr-CA', { timeZone: TIMEZONE, year: 'numeric', month: '2-digit', day: '2-digit' });
    const startMs = new Date(fmt.format(start)).getTime();
    const endMs = new Date(fmt.format(end)).getTime();
...
export function formatSubStep(rang) {
  const sub = rang % 100;
  const step = Math.floor(rang / 100);
...
/** Vérifie si la procédure est clôturée (décret de naturalisation publié → terminée).
 *  Quand c'est le cas, on fige le comptage des durées : plus rien à suivre. */
export function isClosedStatus(statutCode) {
  const code = String(statutCode || '').toLowerCase().trim();
  return ['decret_naturalisation_publie', 'decret_naturalisation_publie_jo', 'decret_publie'].includes(code);
}
